...
@Keep
object SanZong {
    const val keykeyFFF = "2345634129105678"

    const val FCM = "kmrjngrrr"
    fun getConfig(isXS: Boolean = MainStart.mustXS): Config {
        return Config(
            appid = if (!isXS) "h670e13c4e3ab6" else "h67b4300b710bd",
            appkey = if (!isXS) "ac360a993a659579a11f6df50b9e78639" else "a029466d68e3e84efeda8332896ac4ed8",
            openid = if (!isXS) "n1fvmhio0uchmj" else "n1g99srmoblmot",
            upUrl = if (!isXS) "https://test-siemens.wallframeartstyle.com/graphic/feline" else "https://siemens.wallframeartstyle.com/steady/eastman",
            adminUrl = if (!isXS) "https://mata.wallframeartstyle.com/apitest/gght/" else "https://mata.wallframeartstyle.com/api/gght/"
        )
    }
    ...
    fun getAppsflyId(): String {
        return if (!MainStart.mustXS) {
            "5MiZBZBjzzChyhaowfLpyR"
        } else {
            "VFPK6YtLpuMGXfBicGUVDn"
        }
    }
    ...
    const val local_admin_json1 = """
{
  ...
  "resources": {
    "identifiers": {
      ...
      "social": "3616318175247400_/sanfkkt" // FB ID _ 路径（_号分割）
    },
